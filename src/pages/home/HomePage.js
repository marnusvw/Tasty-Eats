...
const HomePage = ({ visibleRecipes, dispatch }) => {
  ...
  return (
    <div className={styles.HomePage}>
      {isLoading ? (
        <p>Hello World, we're waiting for some data</p>
      ) : (
        visibleRecipes?.map((recipe) => <p>{recipe.title}</p>)
      )}

      <RecipeCard
        recipe={recipesMock}
        summary={recipesMock.summary}
        imageSrc={recipesMock.image}
        name={recipesMock.title}
        cookTime={recipesMock.readyInMinutes}
        servings={recipesMock.servings}
      />
